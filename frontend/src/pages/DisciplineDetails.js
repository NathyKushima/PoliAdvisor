  ...
  const create_comment = async (parent_commentId, comment_content, discipline_id) => {
    try {
      const [commentRes] =  await Promise.all([
        axios.post(`http://127.0.0.1:8000/api/create-comment/`, {'comment_content': comment_content, 'parent_comment' : parent_commentId,
  ...
  const report_comment = async (coment_id, denounce_text) => {
    try {
      const [reportRes] =  await Promise.all([
        axios.post(`http://127.0.0.1:8000/api/report-comment/`, {'denounce_text': denounce_text, 'comment_id' : coment_id}, 
